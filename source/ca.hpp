...
#ifndef LITE
#define MAX_CAS                                                      \
    9 /*64 works, but use less for faster comeup.  With              \
...
#define SMOOTHEDGERADIUS                               \
    20 /*How many pixels to smooth on either side of a \
changed edge*/
#define WAVE_PERIOD 200
#define GENERATOR_PERIOD 200
#define OSCILLATOR_BASE_PERIOD 200 // This  many timesteps
#define START_BLT_LINES                                 \
    1 /* 3 is faster, but I don't like the way it looks \
as much.  For "Fastest" mode we do use 3.  But we start in "fast" mode.*/
#define AMPLIFY_VEL_COLOR                                                                 \
    (Real) 80.0 /* 2017 to make the velocity values show up better in the View | Velocity \
mode, see ca.cpp CA:WaveUpdateStep */
#define AMPLIFY_VEL_COLOR_2D                                                \
    (Real) 0.5 /* 2017 to adjust the variable[1] values which are sometimes \
...
#define MEMORY                                                     \
    16 /*This must be >= 2.  You need to be careful if it          \
...
struct Wavecell // a struct is an all-public class
{
    ...
    Real variable[VARIABLE_COUNT]; // intensity if variable[0].
                                   //  In 1d User rules with "RD" or reaction and diffusion, you might have activator
                                   //  and inhibitor in slots 0 and 1. Or wave rules with boiling might use velocity[1]
                                   //  Commented in 2017
...
public:
    ...
    float Amplitude()
    {
        return percent_max_intensity;
    }
    void Amplitude(float newamp)
    {
        (newamp > 1.0 || newamp < 0.0) ? percent_max_intensity = 1.0 : percent_max_intensity = newamp;
    }
    int Location()
    {
        return location;
    ...
    void Location_x(int newlocation)
    {
    ...
    void Location_y(int newlocation)
    {
...
public:
    ...
    {
        return generator[i].g_intensity;
    }
    int Location(int i)
    {
        return generator[i].location;
    } // 1D CA
    int Location_x(int i)
    {
        return generator[i].location_x;
    } // 2D CA
    int Location_y(int i)
    {
        return generator[i].location_y;
    } // 2D CA
};
...
extern "C" DllExport void USERRULE_9(CA *, int, int, int, int, int, int, int, int, int); // 5 1D or 2D nabes
//============================CA CLASS==============================
...
class CA
{
    ...
    friend void Version_123_Load_Individual(ifstream &ifs, CA *target, int version);
...
private:
    ...
    BOOL tp_graphflag, tp_approxflag, tp_cosineflag, tp_sineflag, tp_spectrumflag;
    ...
    Real _wavespeed, _dt_2, _dx_2, _dt_over_2, _dt_over_6, _dt_over_dx_2, _wavespeed_2_times_dt_2_over_dx_2,
        _dt_over_12_times_dx_2, _wavespeed_2_times_dt_2_over_12_times_dx_2, _1_over_mass, _dt_over_mass,
        _wavespeed_2_times_dt_over_dx_2, _dt_2_over_mass;
...
public:
    ...
    bool HasImageBuffer2D()
    {
        return _castyle == CA_WAVE_2D || _castyle == CA_HEAT_2D || _castyle == CA_CUSTOM_2D;
    }
    ...
    long BumpAndReturnGenerationCount()
    {
    ...
    int HorzCount2D()
    {
        return horz_count_2D;
    }
    int VertCount2D()
    {
        return vert_count_2D;
    }
    int Get_TestPoint()
    {
        return test_point;
    ...
    int Get_monochromeflag()
    {
        return monochromeflag;
    }
    int Getband(void)
    {
        return band_count;
    ...
    int Getentropyflag(void)
    {
        return entropyflag;
    }
    Real Gettargetentropy(void)
    {
        return target_entropy;
    }
    // desired entropy
    Real Getentropy(void)
    {
        return entropy;
    } // actual entropy
    BOOL Getgeneratorflag(void)
    {
        return generatorflag;
    }
    Real Getfailstripe(void)
    {
        return fail_stripe;
    }
    Real Getscore()
    ...
    void Setscore(Real val)
    {
        score = val;
    }
    Real Getentropybonus(void)
    {
        return entropy_bonus;
    }
    //----------------CA-Fourier Accessors-----------------------
    int Get_tpviewmode(void)
    {
        return tp_viewmode;
    }
    int Get_tpviewtype(void)
    {
        return tp_viewtype;
    }
    int Get_tpapproxtype(void)
    {
        return tp_approxtype;
    }
    BOOL Get_fourierflag(void)
    {
        return fourierflag;
    }
    int Get_numofterm(void)
    {
        return numofterm;
    }
    BOOL Get_graphflag()
    ...
    BOOL Get_approxflag()
    {
        return tp_approxflag;
    }
    BOOL Get_cosineflag()
    {
        return tp_cosineflag;
    }
    BOOL Get_sineflag()
    {
        return tp_sineflag;
    }
    BOOL Get_spectrumflag()
    {
        return tp_spectrumflag;
    }
    //==============MUTATORS=====================================
    ...
    void Setshowvelocity(int newshowvelocity)
    {
        showvelocity = newshowvelocity;
    }
    ...
    {
        FourierSeed();
    }; // Choose default Seed behaviour here.
    void RandomSeed();
    ...
    void Set_justloadedcells(BOOL onoff)
    {
        _justloadedcells = onoff;
    }
    void One_smoothsteps()
    {
        _smoothsteps = MINSMOOTHSTEPS;
    }
    ...
    void Setentropyflag(BOOL onoff)
    {
        entropyflag = onoff;
        if (entropyflag)
            Resetfreq();
    ...
    //----------------CA-Fourier Mutators-----------------------
    void Set_tpviewmode(int new_viewmode)
    {
        tp_viewmode = new_viewmode;
    }
    void Set_tpviewtype(int new_viewtype)
    {
        tp_viewtype = new_viewtype;
    }
    void Set_tpapproxtype(int new_atype)
    {
        tp_approxtype = new_atype;
    }
    void Set_fourierflag(BOOL num)
    {
        fourierflag = num;
    }
    void Set_TP(void);
    void Set_TempTP(void)
    {
        temp_test_point = test_point;
    }
    void Change_TempTP(int);
    void Set_numofterm(int nt)
    {
        numofterm = nt;
    }
    ...
    void Change_approxflag()
    {
        tp_approxflag = 1 - tp_approxflag;
    }
    void Change_cosineflag()
    {
        tp_cosineflag = 1 - tp_cosineflag;
    }
    void Change_sineflag()
    {
        tp_sineflag = 1 - tp_sineflag;
    }
    void Change_spectrumflag()
    {
        tp_spectrumflag = 1 - tp_spectrumflag;
    }
    void Change_graphflag()
    {
        tp_graphflag = 1 - tp_graphflag;
    }
    //=========================METHODS=================
    ...
    void Locate(int itile_number = 0, int dmaxx = 0, int dmaxy = 0, int CA_count_per_edge = 1);
...
//--------------CA-Oscillator Helper--------------------------------------
    ...
    DerivPair DiverseOscDerivs(
        Real intensity, Real velocity, Real dt, Real massparam, Real frictionparam, Real springparam);
#endif // RUNGE_KUTTE
    ...
    void Wave2D(int, int, int, int, int);
    void Heat2D(int, int, int, int, int);
    void Smooth2D(int, int, int, int, int);
    ...
    friend DllExport void USERRULE_9(CA *, int, int, int, int, int, int, int, int, int);
...
public:
    ...
    {
        return focus;
    } // This is a pointer to the focus CA
    int get_focus_index();
    CA *GetCA(int i)
    {
        return list[i];
    ...
    int Getdimension()
    {
        return list[0]->Getdimension();
    }
    int Getshowvelocity()
    {
        return FocusCA()->showvelocity;
    }
    BOOL Getzoomflag()
    {
        return zoomflag;
    ...
    {
        return sleep;
    } // Use with IDM_PAUSE in capow.cpp
    int GetGlSleep()
    {
        return gl_sleep;
    } // Use with IDC_OPENGL_GLSLEEP
    BOOL Get_justloadedcells()
    {
        return _justloadedcells;
    }
    int WidthCADisplayWindow()
    {
        return (focus->Maxx() - focus->Minx());
    }
    int count_per_edge()
    {
        return CA_count_per_edge;
    }
    ...
    int Getbreedingsteps()
    {
        return breedcycle;
    } // Breed after this many cycles
    BOOL Getbreedflag()
    {
        return breedflag;
    }
    // Breed cycle enabled?
    BOOL Getstripekillflag(void)
    {
        return stripekillflag;
    }
    BOOL Getstripeseedflag(void)
    {
        return stripeseedflag;
    }
    Real Getmutation()
    {
    ...
    void initizlize_blt_lines(int linecount)
    {
    ...
    void SetSleep(BOOL onoff)
    {
        sleep = onoff;
    } // Overriding concern is to PAUSE
    void ToggleSleep()
    {
        sleep ^= 1;
    } // Ditto
    void ToggleGlSleep()
    {
        gl_sleep ^= 1; /*if(!sleep)gl_sleep = 0;*/
    }
    ...
    void Reset_tpgraph();
    int Get_tpdowhich(void)
    {
        return tp_dowhich;
    }
    void Set_tpdowhich(int new_dowhich)
    {
        tp_dowhich = new_dowhich;
    }
    //---------------CAlist Evolution Mutators------------------
    void Setentropy(BOOL onoff); /* 0 for off, 1 for on */
    void Setstripekillflag(BOOL onoff)
    {
        stripekillflag = onoff;
    }
    void Setstripeseedflag(BOOL onoff)
    {
        stripeseedflag = onoff;
    }
    void Setmutation(int updown);
    void Setmutateflag(BOOL imutate)
    {
        mutateflag = imutate;
    }
    void Setbreedflag(BOOL ibreed)
    {
        breedflag = ibreed;
    }
    ...
    void SetEvolve(BOOL newevolve)
    {
        evolveflag = newevolve;
    }
    //===============CAlist Methods================
