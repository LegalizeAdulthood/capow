...
CAlist::CAlist(HWND myhwnd, int maxcount)
{
    ...
    hwnd = myhwnd;
    _plgpl = (LOGPALETTE *) new char[sizeof(LOGPALETTE) + sizeof(PALETTEENTRY) * 236];
    ...
    for (i = 0; i < maxcount; i++)
    {
        ...
        {
            MessageBoxA(
                hwnd, (LPSTR) "Failure in CA Allocation!", (LPSTR) "Memory Problems!", MB_OK | MB_ICONEXCLAMATION);
...
{ // This is called from update_and_show every breeding cycle
    ...
    for (i = field * 3; i < count - 1; i++)
    {
        ...
        case CLONE:
            /* copy a winner */
            focus->CopyCA(list[best_list[Random((unsigned short) field)]]);
            break;
        case SEX:
            ...
            if (field > 1) // more than one winner to be parents?
                while (mother == father)
                    ...
                    father = Random((unsigned short) field);
            focus->Become_child_of(list[best_list[mother]], list[father]);
            break;
        case NONE:
            break;
        }
        ...
        for (i = 0; i < count; i++) /* now test it with the rest of the unclaimed
            ...
            if (used[i] == 0)
            {
                if (list[i]->score > list[best_list[j]]->score)
                    best_list[j] = i;
                else if (list[i]->score == list[best_list[j]]->score && Random(2)) // To equalize chances of winning.
                    best_list[j] = i;
            }
...
BOOL CAlist::GetOpenGLDisplayRect(RECT *rect)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (ca->viewmode == IDC_2D_VIEW && ca->HasImageBuffer2D() && ca->wavePlaneImage.Data() != 0)
            converted = TRUE;
        if ((ca->viewmode == IDC_DOWN_VIEW || ca->viewmode == IDC_SCROLL_VIEW || ca->viewmode == IDC_GRAPH_VIEW ||
                ca->viewmode == IDC_SPLIT_VIEW) &&
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {                                    // Begin randomizing the type
            ...
            if (type == CA_STANDARD || type == CA_REVERSIBLE)
            {
                ...
                list[i]->Changeradiusandstates(StartRadius[stateradiusindex], StartStates[stateradiusindex]);
